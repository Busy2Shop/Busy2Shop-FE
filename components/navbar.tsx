...
export default function Navbar() {
    ...
    return (
        <>
                ...
                <div className="max-w-[1440px] mx-auto px-2 sm:px-4">
                    <div className="flex items-center justify-between h-16 sm:h-20">
                        ...
                        <div className="flex items-center gap-2 sm:gap-4">
                            <Link href="/" className="flex items-center gap-2">
                                <Image src="/images/logo-for-white-bg.png" alt="Logo" width={28} height={28} className="rounded-full sm:w-9 sm:h-9" />
                                <span className="text-[#00A67E] text-lg sm:text-2xl font-bold tracking-tight">
                                    Busy
                        ...
                        <button
                            className="md:hidden flex items-center justify-center p-2 rounded focus:outline-none focus:ring-2 focus:ring-[#00A67E]"
                            onClick={() => setIsMenuOpen((v) => !v)}
                            aria-label="Open menu"
                        >
                            <svg className="w-7 h-7 text-[#00A67E]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                        ...
                        <div className="flex-1 max-w-2xl mx-4 relative hidden md:block">
                            ...
                            {showSearchSuggestions && searchQuery && (
                                <div 
                                    className="absolute left-0 right-0 mt-1 bg-white rounded-md shadow-lg border py-2 z-50"
                    ...
                    {isMenuOpen && (
                        <div className="fixed inset-0 z-50 bg-black/40" onClick={() => setIsMenuOpen(false)}>
                            <div className="absolute top-0 left-0 w-4/5 max-w-xs h-full bg-white shadow-lg p-4 flex flex-col gap-4 animate-slide-in">
                                <div className="flex items-center justify-between mb-2">
                                    <Link href="/" className="flex items-center gap-2" onClick={() => setIsMenuOpen(false)}>
                                        <Image src="/images/logo-for-white-bg.png" alt="Logo" width={28} height={28} className="rounded-full" />
                                        <span className="text-[#00A67E] text-lg font-bold tracking-tight">
                                            Busy<span className="text-[#FF6B00]">2</span><span className="text-[#00A67E]">Shop</span>
                                        </span>
                                    </Link>
                                ...
                                <Link href="/help" className="flex items-center gap-2 py-2 text-gray-700 hover:text-[#00A67E]" onClick={() => setIsMenuOpen(false)}>
                                    <HelpCircle className="h-5 w-5" /> Help
                                </Link>
                                <Link href="/cart" className="flex items-center gap-2 py-2 text-gray-700 hover:text-[#00A67E]" onClick={() => setIsMenuOpen(false)}>
                                    <ShoppingCart className="h-5 w-5" /> Cart
                                    <span className="ml-2 bg-[#FF6B00] text-white text-xs rounded-full h-5 w-5 flex items-center justify-center font-bold">5</span>
                                </Link>
                                <Link href="/profile" className="flex items-center gap-2 py-2 text-gray-700 hover:text-[#00A67E]" onClick={() => setIsMenuOpen(false)}>
                                    <User2 className="h-5 w-5" /> Profile
                                </Link>
                                <Button variant="outline" className="w-full border-[#00A67E] text-[#00A67E] mt-2" onClick={() => setIsMenuOpen(false)}>
                                    <Store className="h-4 w-4 mr-2" /> Become an Agent
                                </Button>
                            </div>
                        </div>
                    )}
            ...
            {/* Search Bar only visible below navbar on mobile */}
            <div className="block md:hidden w-full bg-white border-b shadow-sm px-2 sm:px-4 py-1 sticky top-[4rem] z-40">
                <div className="max-w-[900px] mx-auto">
                    <form className="relative">
                        ...
                        <div className="relative flex items-center bg-white border border-gray-300 rounded-md overflow-hidden focus-within:ring-2 focus-within:ring-[#00A67E] focus-within:border-[#00A67E]">
                            <Search className="absolute left-2 text-gray-400 h-4 w-4" />
                            <Input
                                id="search-mobile"
                                value={searchQuery}
                                onChange={(e) => {
                                ...
                                onFocus={() => setShowSearchSuggestions(true)}
                                className="pl-8 pr-2 py-2 w-full border-0 focus:ring-0 text-sm"
                                placeholder="Search products, brands and categories"
                            />
                            <Button
                                type="submit"
                                className="h-8 px-3 bg-[#FF6B00] hover:bg-[#ff8c42] text-white text-sm font-semibold transition-colors duration-200 rounded-none"
                            >
                                Search
                            </Button>
                        </div>
